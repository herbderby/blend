...
void check_n(struct stage* stage, void* vdst, __m128 d, __m128 s, int n) {
    if (n--) {
        stage->next->fn(stage->next, vdst,d,s,n);
    }
}

void load_srgb_dst(struct stage* stage, void* vdst, __m128 d, __m128 s, int n) {
    int* dst = vdst;
...
void load_srgb_src(struct stage* stage, void* vdst, __m128 d, __m128 s, int n) {
    int* src = stage->ctx;
    stage->ctx = src+1;
...
void srcover(struct stage* stage, void* vdst, __m128 d, __m128 s, int n) {
...
void lerp_a8_cov(struct stage* stage, void* vdst, __m128 d, __m128 s, int n) {
    char* cov = stage->ctx;
    stage->ctx = cov+1;
...
void store_srgb(struct stage* stage, void* vdst, __m128 d, __m128 s, int n) {
    int* dst = vdst;
    vdst = dst+1;
