...
void load_srgb_dst(const struct stage* stage, size_t n, __m128 d, __m128 s) {
    int* dst = stage->ctx;
    ...
    stage->next->fn(stage->next, n,d,s);
}

void load_srgb_src(const struct stage* stage, size_t n, __m128 d, __m128 s) {
...
void srcover(const struct stage* stage, size_t n, __m128 d, __m128 s) {
...
void lerp_a8_cov(const struct stage* stage, size_t n, __m128 d, __m128 s) {
...
void store_srgb(const struct stage* stage, size_t n, __m128 d, __m128 s) {
    int* dst = stage->ctx;
