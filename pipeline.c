...
static bool shortcircuit_srcover_both_rgba8888_(const void* ctx, size_t x, void* dp,
    ...
    uint32_t* dst = dp;
    switch (src[x] >> 24) {
        case 255: dst[x] = src[x]; return false;
        case   0:                  return false;
    }
    return true;
}
...
void fused(uint32_t* dst, const uint32_t* src, const uint8_t* cov, size_t n) {
    __m128 d = _mm_undefined_ps(),
           s = _mm_undefined_ps();
    for (size_t x = 0; x < n; x++) {
        load_d_srgb_(NULL, x,dst,&d,&s);
        load_s_srgb_( src, x,dst,&d,&s);
            srcover_(NULL, x,dst,&d,&s);
            lerp_u8_( cov, x,dst,&d,&s);
       store_s_srgb_(NULL, x,dst,&d,&s);
    }
}
